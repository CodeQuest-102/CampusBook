...
import { Screen, TopBar, Avatar, Button } from '../../components';
import { colors, fontWeight, radius, shadow, spacing, typography } from '../../theme';
import { pendingRequests, BookingRequest } from '../../data/placeholder';
import type { RootStackParamList } from '../../navigation/types';

type Nav = NativeStackNavigationProp<RootStackParamList>;

const TABS = [
  { key: 'all', label: 'All', count: pendingRequests.length },
  { key: 'staff', label: 'Staff', count: pendingRequests.filter((r) => r.requesterRole === 'Staff').length },
  { key: 'students', label: 'Students', count: pendingRequests.filter((r) => r.requesterRole === 'Student').length },
];
    ...
    <View style={styles.card}>
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.sub} numberOfLines={1}>
          {request.building} ({request.roomName.replace(/[^A-Z0-9.]/gi, '').slice(-4) || 'Room'})
        </Text>
